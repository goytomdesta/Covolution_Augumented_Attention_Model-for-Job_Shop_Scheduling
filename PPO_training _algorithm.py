...
from Utilities.agent_utils import eval_actions
from Utilities.agent_utils import select_action
...
class PPO:
    def __init__(self,
        ...
        #
        # self.policy = nn.DataParallel(self.policy, device_ids=[0,1])
        ...
        # pis,vals = torch.nn.DataParallel(pis,vals)
        # pis,vals = pis,vals.cuda()
    ...
    def update(self, memories, n_tasks, g_pool):
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(len(memories)):
                ...
                # pis,vals = torch.nn.DataParallel(pis,vals)
                # pis,vals = pis,vals.cuda()
                # self.policy = self.policy("x", "graph_pool", "adj", "candidate", "mask", "device", "padded_nei")
                # self.policy = nn.DataParallel(self.policy)
                # self.policy.to(device)
...
def main():
    ...
    for i_update in range(configs.max_updates):
        ...
        # mean_rewards_all_env = -10 * math.log(sum(1 / ep_rewards[j]*ep_rewards[j]))
        ...
        # # import torch
        #
        # print(torch.cuda.is_available()) # To check whether GPU is available
        # print(torch.cuda.current_device()) # to check currently being used GPU id
        # print(torch.cuda.device_count()) # number of installed GPU device
        #
